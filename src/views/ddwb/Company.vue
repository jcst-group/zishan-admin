<template>
    <div>
        <div style="margin-bottom:10px;">
            ...
            <a-button  style="margin-right:15px;" type="primary" icon="edit" @click="addpic1">添加后台首页背景图</a-button>
            <a-button  style="margin-right:15px;" type="primary" icon="edit" @click="handleAdd">编辑icon</a-button>
        ...
        <a-table :pagination="false" :rowKey="record => record.id" :columns="columns" :dataSource="data" :loading="loading" bordered>
            ...
            <span slot="ation1" slot-scope="text, record" style="white-space: nowrap">
        ...
        >
         <div>后台首页背景图：</div>
                <div style="height:100px;margin:20px 0 20px 0;">
                    <a-upload
                        name="file"
                        action="/api2/platform-info/addBackHomePic"
                        listType="picture-card"
                        :fileList="mpfileList"
        ...
        <changelogo ref='changelogo'></changelogo>
    </div>
</template>
<script>
import Vue from 'vue'
import { ACCESS_TOKEN } from "@/store/mutation-types"
import { getCompanyList,delCompany,getFertilizersList,delFertilizers,delPic,delFile,getTeamList,delTeam,getCompanyExpect,updateCompanyExpect,delCompanyExpect,getBackPicName } from '@/api/ddwbApi'
...
import Platform from '@/views/ddwbmodule/platform'
import changelogo from '@/views/ddwbmodule/Addlogo'
...
export default {
    ...
    methods:{
        handleAdd() {
            this.$refs.changelogo.title = '编辑icon'
            this.$refs.changelogo.open()
        },
        hmsbbb(){
            getBackPicName().then(res=>{
                if(res.success){
                    this.hmsb=res.result;
                    if(this.hmsb){
                        this.mpfileList=[];
                            let pic=this.hmsb;
