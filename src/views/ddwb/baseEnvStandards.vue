<template>
    <div class="temp">
        ...
        <div class="input">
            <span class="title">检测类型：</span>
           <a-select v-model="condition" class="inp">
              <a-select-option v-for="(v,k) in Condition" :value="v.value" :key="k">{{v.displayName}}</a-select-option>
            </a-select>
        </div>
        <div class="btable">
...
export default {
  ...
  methods:{
     handleTableChange(pagination) {
      ...
      search(){
        if(this.condition){
          this.par.standardTypeId=this.condition;
        }
       this.loadData();
      },
  },
